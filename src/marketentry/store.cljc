(ns marketentry.store
  "SSoT for the AUT market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems AT" :portal "USP.gv.at"
             ...
             :requires-firmenbuch? true :firmenbuch-verified? true
             :drafted? false :submitted? false
             :jurisdiction "AUT" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "USP.gv.at"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-firmenbuch? true :firmenbuch-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "USP.gv.at"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "AUT" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "USP.gv.at"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "AUT" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "USP.gv.at"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-firmenbuch? true :firmenbuch-verified? false
             :drafted? false :submitted? false
             :jurisdiction "AUT" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "USP.gv.at"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               ...
                               requires-firmenbuch? firmenbuch-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-firmenbuch?)    (assoc :engagement/requires-firmenbuch? requires-firmenbuch?)
    (some? firmenbuch-verified?)    (assoc :engagement/firmenbuch-verified? firmenbuch-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-firmenbuch? :engagement/firmenbuch-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-firmenbuch? (boolean (:engagement/requires-firmenbuch? m))
     :firmenbuch-verified? (boolean (:engagement/firmenbuch-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-aut"})
