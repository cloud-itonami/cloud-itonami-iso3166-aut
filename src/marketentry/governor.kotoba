(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Austria-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Austria-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Austria
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(FIRMENBUCH/統一資格/USP.gv.at登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Austria public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はオーストリア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- firmenbuch-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-firmenbuch? true`, INDEPENDENTLY check
  `:firmenbuch-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-firmenbuch? e))
                 (not (true? (:firmenbuch-verified? e))))
        [{:rule :firmenbuch-unverified
          :detail (str subject " はFIRMENBUCH確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (firmenbuch-unverified-violations request st)
