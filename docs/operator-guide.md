...
2. Register the client's intake: business type, target public function,
   prior filing history in Austria if any.
3. Run the advisor in read-only mode against the USP (Unternehmensserviceportal, usp.gv.at) eProcurement search under BVergG 2018, with federal central purchasing via BBG (Bundesbeschaffung GmbH).
4. Compare the checklist against the client's current documentation
   (Firmenbuch commercial-register extract plus UID (Umsatzsteuer-Identifikationsnummer) via FinanzOnline / USP).
...
- named referral relationship with Austrian-licensed counsel or a
...
a working referral relationship with Austrian-licensed counsel or a
registered agent for whatever licensed representation the law of
Austria requires for actual public-procurement filings.
